...
![Simulators](https://img.shields.io/badge/simulators-8%20Backends-orange)
...
![Tests](https://img.shields.io/badge/tests-208%2B%20PASS-brightgreen)
...
|---------|------|--------|----------|
| **IBM Quantum** | Superconducting | ✅ Verified on ibm_fez | qiskit-ibm-runtime adapter, noise model, transpiler |
...
**Note:** "SDK-wired" means adapter code exists that calls the vendor's own SDK. These have NOT been tested against real quantum hardware. Only IBM, D-Wave, and SpinQ have been verified against simulated/real environments.
...
|----------|--------|-------|
| **Python** | ✅ Complete | Primary SDK, full feature set (208+ tests) |
...
| **v0.7.0** | 2026-07 | **Fault-Tolerant QEC** — Stabilizer/Surface/Color codes, 5 decoders, magic state distillation, LDPC codes — 83 tests |
| **v0.6.0** | 2026-07 | **Quantum Communication** — 7 protocols: BB84, E91, CV-QKD, DI-QKD, satellite, repeaters, network — 30 tests |
| **v0.4.0** | 2026-07 | **Novel contributions** — Noise-Adaptive Compiler, SPAE, Circuit Cutting, Hybrid MPS-Clifford Simulator |
| **v0.3.0** | 2026-06 | QuantumRun primitives, QNN, 6 domain modules, Unitary Synthesis, Adaptive Error Mitigation, Pulse Translation, Dynamic Circuits |
| **v0.2.0** | 2026-05 | Quantum OS, Post-Quantum Security, 3 simulation backends, circuit library, visualization |
...
## Desktop GUI — "VS Code for Quantum Computing"

A full-featured quantum IDE built with **Tauri 2.x** (Rust + React + TypeScript). Runs natively on Linux, macOS, and Windows with a 13MB binary.
...
**Circuit Editor:**
- Drag-and-drop gate placement on HTML5 Canvas
- 14 quantum gates: H, X, Y, Z, S, T, Rx, Ry, Rz, CNOT, CZ, SWAP, Toffoli, Measure
- Color-coded gates with glow effects, multi-qubit wire rendering
- Right-click context menu (delete, copy gates)
- Grid dots for precise alignment
- Qubit count selector (1–20 qubits)

**Code Editors:**
- **Python Editor** — Monaco (VS Code engine) with quantum Python syntax highlighting, auto-complete for gate keywords
- **OpenQASM 2.0 Editor** — dedicated QASM syntax highlighting with bidirectional parse: QASM → Circuit and Circuit → QASM
- **Parse to Circuit** button converts code to visual circuit in real-time

**Framework Integration — Run on ANY backend:**
- **AbirQu Simulator** — native NumPy statevector simulator
- **Qiskit** — converts circuit to Qiskit format, runs on Aer simulator (or falls back to NumPy)
- **Cirq** — converts to Cirq, runs on Google's simulator (or falls back to NumPy)
- **OQTOPUS** — OQTOPUS cloud interface (or falls back to NumPy)
- **D-Wave** — quantum annealing simulation, QUBO/Ising conversion
- Framework availability detector — shows which SDKs are installed
- One-click "Run on [Framework]" buttons with real-time results

**Resizable Panels:**
- Drag-to-resize split panels (horizontal + vertical)
- Left sidebar (hardware/jobs/noise), center editor, right panel, bottom console
- Collapse/expand right panel with toggle
...
- **Measurement Histogram** — shot distribution + per-qubit bias (|0⟩/|1⟩ percentages) + Shannon entropy

**Noise Simulation:**
- Enable/disable noise model toggle
- 4 noise parameters: depolarizing, amplitude damping, phase damping, readout error
- Slider controls for each parameter
- Preset noise profiles: Noiseless, IBM Nairobi, Google Sycamore, Heavy Noise
- Visual "Noise ON" indicator in toolbar

**Export — Research Reports:**
- **HTML Report** — full research report with circuit, stats (qubits, gates, shots), measurement table, OpenQASM code
- **PDF (via HTML)** — opens report in browser, triggers Print to PDF
- **OpenQASM 2.0** — industry-standard quantum assembly export
- **JSON** — structured circuit + results data

**Hardware Management:**
- 6 providers grouped: IBM Quantum, D-Wave, AbirQu Simulators
- Status indicators (online/offline/simulated)
- Qubit count, backend type (real/simulator/hybrid)
- One-click backend selection

**Job Dashboard:**
- Real-time job monitoring with progress tracking
- Job history with status (pending/running/completed/failed)
- Active job indicator in toolbar

**Circuit Library:**
- 12 built-in templates: Bell State, GHZ, Grover, QFT, Teleportation, VQE, QAOA, Bernstein-Vazirani, Quantum Walk, Simon's, W State, Random
- Search, category filtering, difficulty levels
- Load template directly into circuit editor

**Console:**
- Real-time output display with line numbers
- Color-coded output (info/error/success)
- Auto-scroll to latest

**Dark/Light Themes:**
- Instant switching via CSS variables
- Glassmorphism design with backdrop blur
- Gradient buttons, glow effects, smooth animations
...
### v1.2.0 (Full Quantum IDE)
- **Full desktop IDE**: Tauri 2.x (Rust shell + React/TypeScript frontend)
- **Circuit Canvas**: HTML5 Canvas with drag-drop gate placement, 14 gates, color-coded with glow
- **Python Editor**: Monaco (VS Code) with quantum Python syntax highlighting
- **OpenQASM 2.0 Editor**: Dedicated QASM syntax with bidirectional parse (QASM ↔ Circuit)
- **Framework Integration**: Run circuits on AbirQu/Qiskit/Cirq/OQTOPUS/D-Wave with one click
- **Bloch Sphere**: Interactive 3D with drag-to-rotate, auto-rotate, great circles, state arrow
- **Results Panel**: Measurement histogram + per-qubit bias + Shannon entropy + state vector bars
- **Resizable Panels**: Drag-to-resize split panels (horizontal + vertical) for custom layouts
- **Noise Simulation**: Depolarizing/amplitude/phase/readout noise with presets (IBM, Google, Heavy)
- **Export Reports**: HTML research reports, PDF (via browser print), OpenQASM, JSON export
- **Hardware Panel**: 6 providers (IBM, D-Wave, AbirQu) with status indicators
- **Job Dashboard**: Real-time job monitoring with progress bars and history
- **Circuit Library**: 12 built-in templates with search and category filtering
- **Console**: Real-time output with color-coded lines
- **Dark/Light Themes**: CSS variable-based glassmorphism design with animations
- **Cross-platform installers**: .deb, .rpm, .AppImage (Linux), .dmg (macOS), .exe (Windows)
- **Binary size**: 13MB (release), 4MB (.deb/.rpm)
- **Backend IPC**: 20 Tauri commands wired to Python subprocess via JSON-over-stdio
